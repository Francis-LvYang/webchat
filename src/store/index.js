...
const store = new Vuex.Store({
  ...
  actions: {
    ...
    async clearAllMessHistory({state, commit}, data) {
      const res = await url.RoomHistoryCleanAll(data);
      if (res.data.data.errno === 0) {
    ...
    async clearOneMessageHistroy({state, commit}, data) {
      await url.RoomHistoryCleanOneMessage(data);
    },
