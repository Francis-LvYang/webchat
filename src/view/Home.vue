    ...
    <div class="content">
      <mu-list>
        <mu-list-item title="修改头像" @click="changeAvatar">
          <mu-icon slot="left" value="send" color="green"/>
        </mu-list-item>
        <mu-list-item title="赞助一下" @click="handleTips">
          <mu-icon slot="left" value="inbox" color="primary"/>
        </mu-list-item>
        ...
        <mu-list-item title="添加好友" @click="searchOther">
          <mu-icon slot="left" value="search" color="red"/>
        </mu-list-item>
      </mu-list>
      <!--<mu-divider/>-->
    </div>
...
import {deleteCacheById} from '../../server_modules/cache';
...
export default {
  ...
  methods: {
    ...
    async rmLocalData() {
      ...
      const username = `${this.userid}-room2`;
      const userinfo = `${this.userid}-room1`;
      await deleteCacheById(userinfo);
      await deleteCacheById(username);
      if (data === "submit") {
